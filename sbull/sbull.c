...
#include <linux/kernel.h>	   /* printk()：内核打印 */
#include <linux/slab.h>		   /* kmalloc()：内核内存分配 */
#include <linux/fs.h>		   /* 文件系统相关操作 */
#include <linux/errno.h>	   /* 错误码定义 */
#include <linux/timer.h>	   /* 内核定时器 */
#include <linux/types.h>	   /* 类型定义（size_t等） */
#include <linux/fcntl.h>	   /* 文件打开模式（O_ACCMODE等） */
#include <linux/hdreg.h>	   /* HDIO_GETGEO：获取磁盘几何信息 */
#include <linux/kdev_t.h>	   /* 设备号操作 */
#include <linux/vmalloc.h>	   /* vmalloc()：虚拟内存分配（大内存） */
#include <linux/genhd.h>	   /* gendisk：通用磁盘结构 */
#include <linux/blkdev.h>	   /* 块设备核心接口 */
#include <linux/buffer_head.h> /* invalidate_bdev： invalidate缓冲区 */
#include <linux/bio.h>		   /* bio：块I/O操作结构 */

MODULE_LICENSE("Dual BSD/GPL"); /* 模块许可证：双BSD/GPL */

static int sbull_major = 0;			 /* 主设备号（默认动态分配） */
module_param(sbull_major, int, 0);	 /* 模块参数：允许用户指定主设备号 */
static int hardsect_size = 512;		 /* 硬件扇区大小（字节） */
module_param(hardsect_size, int, 0); /* 模块参数：硬件扇区大小 */
static int nsectors = 1024;			 /* 磁盘扇区总数（模拟磁盘大小） */
module_param(nsectors, int, 0);		 /* 模块参数：扇区数量 */
static int ndevices = 4;			 /* 设备数量 */
module_param(ndevices, int, 0);		 /* 模块参数：设备数量 */
...
{
	RM_SIMPLE = 0,	/* 简单请求处理 */
	RM_FULL = 1,	/* 完整请求处理（支持请求合并） */
	RM_NOQUEUE = 2, /* 使用自定义make_request函数 */
};
static int request_mode = RM_SIMPLE; /* 默认简单请求模式 */
module_param(request_mode, int, 0);	 /* 模块参数：请求模式 */
